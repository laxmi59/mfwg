...
if($_POST['action'] =='DeletePurchase'){
	...
	echo '<div id="AddItem_formdiv">
<div class="smpopup" style="height:250px">
<h1><span>Cancel PAYMENT</span></h1>
<div class="pd40"> 
<p class="qs" >Are you sure you want to cancel this payment?</p>
<div><a href="view-purchases.php?action=delete&id='.$id.'" class="btn-yes"> Yes</a><a href="" class="btn-no">NO</a></div>
</div>
</div>
 </div>';
...
if($_POST['action']=='editEvent')
{
	...
	$content="<div id='AddItem_formdiv' style='display:'><form id='edit_event' method='post' action=''><input type='hidden' id='id_edit' name='id' value='".$id."' /><div class='additem'><h1><span>Edit time</span> SEtting</h1><ul><li><label>Title</label> <input id='title_edit' name='title'  class='input' value='".$event_title."'> </li><li><label>Day of Week</label><select name='day' id='day_edit' >";
foreach($days as $k=>$v)
{
//if($k>=date('w'))
{
...
$content.='</select> </li>
<li><input type="button" onclick="return edit_events();" class="btn-save-sm" name="insertItem" value="save"> </li>
</ul>
</div>
  </form></div>';
echo $content;
...
if($_POST['action'] =='Deleteevent'){
	...
	echo '<div id="AddItem_formdiv">
<div class="smpopup" style="height:250px">
<h1><span>Delete</span> Item</h1>
<div class="pd40">  
<p class="qs" >Are you sure you want to delete this item?</p>
<div><a href="my-calendar.php?action=delete&id='.$id.'" class="btn-yes"> Yes</a><a href="my-calendar.php" class="btn-no">NO</a></div>
</div>
</div>
 </div>';
...
if($_POST['action']='checkevent')
{
extract($_POST);
$data=$S->checkEvent($id,$day,$start_time,$end_time);
echo $data; exit;
if($data=='true')
{
 echo 'true';
